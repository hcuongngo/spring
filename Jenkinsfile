    ...
    stages {
        ...
        stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh "docker run --name khalid-mysql --rm --network dev -v khalid-mysql-data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=${cutu123#C} -e MYSQL_DATABASE=db_example  -d mysql:8.0 "
                sh 'sleep 20'
                sh "docker exec -i khalid-mysql mysql --user=root --password=${cutu123#C} < script"
            }
        }
